...
public:
	...
	{
		TextLog::Log("DashboardService")<<"Created for path: "<<path;
		if(!AddThread(pusher))
			TextLog::Log("DashboardService", LEVEL_t::ERR)<<"Could not Add a new thread for this pusher ("<<path<<")";
	};

public:
	virtual ~DashboardService() {TextLog::Log("DashboardService", "Destroying");};

public:
	void SetPusher(const shared_ptr<DashboardPusher>& push)
	{
		TextLog::Log("Dashboard Service", "Installing New Pusher");
		pusher = push;
		AddThread(pusher);
	}
...
public:
	...
	const shared_ptr<PushReference<T>> Add(const Frame::KEY_t key, const weak_ptr<PollValue<T>>& poll, const int data=-1)
	{
		...
		if(pushRef == nullptr)
		{
			auto save = TextLog::Log("DashboardService", LEVEL_t::ERR)<<"Add(key: "<<key<<", poll: ";
	...
	const shared_ptr<PushReference<T>> Add(const Frame::KEY_t key, const T *val, const int data=-1)
	{
		...
		if(pushRef == nullptr)
		{
			auto save = TextLog::Log("DashbaordService", LEVEL_t::ERR)<<"Add(key: "<<key<<", val: ";
			if(val == NULL)
				save<<="NULL";
			else
				save<<="non-null";
